        ...
        <div className="w-full space-y-8 lg:w-[60%]">
          <header className="space-y-4">
            <div className="flex flex-wrap gap-1.5">
              {tags.map((tag) => <Badge key={tag} tone="accent" mono>{tag}</Badge>)}
            </div>
                ...
                {skill.executions.map((exec) => (
                  <li key={exec.id} className="flex items-center justify-between bg-bg-subtle px-4 py-2.5">
                    <span className="flex items-center gap-2.5">
                      ...
                      <span className="font-mono text-small text-text-primary">{exec.walletAddress.slice(0, 4)}…{exec.walletAddress.slice(-4)}</span>
                    </span>
        ...
        <div className="w-full lg:w-[40%]">
          <div className="sticky top-20 space-y-5 rounded-xl border border-border bg-bg-subtle p-6 shadow-sm">
            ...
            <div className="space-y-2 rounded-lg border border-border bg-bg-primary p-3">
              <p className="font-mono text-tiny uppercase tracking-[0.14em] text-text-tertiary">Fee split</p>
              {[
            ...
            {connected ? (
              <Button fullWidth size="lg" variant="accent" onClick={execute} loading={executing} disabled={!input} leadingIcon={!executing ? <BoltIcon size={18} /> : undefined}>
                {executing ? "Executing…" : "Execute skill"}
              </Button>
            ) : (
              <Button fullWidth size="lg" onClick={() => setVisible(true)}>Connect wallet to run</Button>
            )}
